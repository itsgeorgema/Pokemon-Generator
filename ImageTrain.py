...
merged_df= merged_df[merged_df['image_path'].isin(valid_paths)].reset_index(drop=True)
...
class PokemonDataset(Dataset):
    ...
    def one_hot_type(self, t):
        vec=[0]*len(self.types)
        if pd.notna(t) and t in self.types:
            vec[self.types.index(t)]=1
        return vec
    
    def __getitem__(self, idx):
        row=self.df.iloc[idx]
        condition=self.one_hot_type(row['Type1']) + self.one_hot_type(row['Type2']) + [
            row['Height (m)']/self.height_max,
...
def train_gan(generator, discriminator, dataloader, z_dim, condition_dim, num_epochs=100, start_epoch=0, g_opt=None, d_opt=None):
    ...
    for epoch in range(start_epoch,start_epoch+num_epochs):
        ...
        if (epoch+1)%10==0:
            with torch.no_grad():
                ...
                img=sample[0].permute(1, 2, 0).cpu().numpy() *.5+.5
